...
async def lifespan(app: FastAPI):
	...
	try:
		...
		interactive_mapper = InteractiveMapper(llm=llm, config=MapperConfig(headless=False))
...
@app.post('/api/v1/mapping/start', response_model=dict[str, Any])
async def start_mapping(request: StartMappingRequest):
	...
	async def run_mapper():
		...
		try:
			...
			mapper_result, _ = await interactive_mapper.map_objective(map_request, session=session)
...
@app.get('/api/v1/mapping/sessions/{session_id}', response_model=MappingSessionState)
...
@app.post('/api/v1/mapping/sessions/{session_id}/input')
...
@app.get('/api/v1/mapping/sessions/{session_id}/events')
...
@app.post('/api/v1/mapping/sessions/{session_id}/cancel')
...
@app.get('/api/v1/mapping/sessions')
...
@app.delete('/api/v1/mapping/sessions/{session_id}')
...
@app.post('/api/v1/mapping/sessions/{session_id}/create-plan', response_model=Plan)
async def create_plan_from_session(session_id: str, plan_name: str | None = None):
	...
	if not mapper_result.success:
		raise HTTPException(status_code=400, detail=f'Cannot create plan from failed mapping: {mapper_result.error_message}')
...
@app.get('/api/v1/plans/search', response_model=list[Plan])
...
@app.post('/api/v1/execute/{plan_id}', response_model=ExecutionResult)
...
@app.get('/api/v1/executions', response_model=list[ExecutionResult])
